...
function paasta(e) { 
  ...
  kohde.classList.add("afterTransition"); // New
...
function scroll() {
  ...
  if(pohja.offsetHeight + pohja.scrollTop + scrollMaara > pohja.scrollHeight) {
    if(pohja.offsetHeight + pohja.scrollTop < pohja.scrollHeight + 4) aloitusY -= pohja.scrollHeight - pohja.scrollTop - pohja.offsetHeight + 4;
    pohja.scrollBy(0, 100);
    return
  } else if(pohja.scrollTop + scrollMaara <= 0) {
    if(pohja.scrollTop > 0) aloitusY += pohja.scrollTop;
    pohja.scrollBy(0, -100);
    return
  }
...
function poistaTransition(e) {
  setTimeout(() => {
    ...
    e.classList.remove("afterTransition");
  }, 250)
}
